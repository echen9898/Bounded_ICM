...
-- get_level_section - Returns the area you're currently in
function get_level_section()
    return memory.readbyte(addr_curr_page)
end;

function hook_set_start()
    if (get_level_section() ~= 0) then
        memory.writebyte(addr_curr_page, 0);
    end;
end;

function hook_set_life()
    if memory.readbyte(addr_life) ~= 0x01 then
        memory.writebyte(addr_life, 0x01);
    end;
end;
memory.registerwrite(addr_life, hook_set_life)
memory.registerwrite(addr_curr_page, hook_set_start) ------- added by me
